...
class GithubPullRequestsMatcher extends NaturalMatcher {
  ...
  async execute() {
    const { res, repoName: repo, repoOwner: owner, hasRepoName, onlyMine, legacy, text } = this;
    if (!hasRepoName) return null;
    if (legacy) {
      classifier.addDocument(text, GITHUB_PR_PLURAL);
    }

    res.send(`Loading ${onlyMine ? 'your ' : ''}*Pull Requests* for ${owner}/${repo}`);
    ...
    try {
      ...
      const view = {
        ...
        prs: prs.filter(filterByUser(user, onlyMine)).map(addReviewerData),
      };

      if (view.prs.length === 0) {
        return res.send(`There are no open *Pull Requests* on ${owner}/${repo}`);
      }

      return res.send(Mustache.render(pullRequestsTemplate, view));
    } catch (error) {
      logger.error(error);
...
GithubPullRequestsMatcher.keywords = ['pull requests', 'pull request', 'prs', 'pr'];
